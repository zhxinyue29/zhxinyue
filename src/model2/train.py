...
class EmbeddingExtractor:
    ...
    def __init__(self, tokenizer_dir: str, device: torch.device, logger: logging.Logger):
        ...
        try:
            from src.model1.predict import infer_embeddings as _user_infer
            self._user_infer = _user_infer
            self.logger.info("🧩 抽特征：优先使用 src.model1.predict.infer_embeddings")
        except Exception:
